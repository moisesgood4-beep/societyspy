...
- **Simple Mode :**
    ...
    ```
- **Advanced Mode :**
    Activate the interactive setup menu for full customization using the `-A` or `--advanced` flag. This will allow you to configure password length, transformations, and more.
    ```bash
    python passlord.py -A
    ```
...
### Configuration ⚙️
The power of PassLord comes from the `patterns/rules.txt` file. Each line in this file is a rule that tells the script how to combine the keywords you provide.

* **Location:** The script expects to find this file at `patterns/rules.txt`.
* **Syntax:** Keywords are separated by the `@` symbol.
