...
from core.assets.banners import AnonymousLogo
import re as r
import netifaces
...
def safekill():
    ...
    exec_command(
        "bleachbit -o -c bash.history system.cache system.clipboard system.custom system.recent_documents system.rotated_logs system.tmp system.trash adobe_reader.cache chromium.cache chromium.session chromium.history chromium.form_history elinks.history emesene.cache epiphany.cache firefox.cache firefox.crash_reports firefox.url_history firefox.forms flash.cache flash.cookies google_chrome.cache google_chrome.history google_chrome.form_history google_chrome.search_engines google_chrome.session google_earth.temporary_files links2.history opera.cache opera.form_history opera.history /var/log/auth.log /var/log/secure /var/log/syslog /tmp/* /var/tmp/* ~/.bash_history ~/.zsh_history ~/.ssh/known_hosts ~/.local/share/recently-used.xbel ~/.cache/Slack ~/.cache/Teams ~/.cache/vlc ~/.Skype ~/.dropbox ~/.steam ~/.config/opera ~/.config/vivaldi ~/.config/chromium ~/.config/BraveSoftware/Brave-Browser ~/.cache/libreoffice ~/.config/libreoffice ~/.cache/gimp ~/.thumbnails ~/.local/share/Trash ~/.cache/gnome-software /var/log/Xorg.*.log > /dev/null 2>&1"
    )
