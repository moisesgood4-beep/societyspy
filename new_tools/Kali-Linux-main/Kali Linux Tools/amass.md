![amass.png](https://github.com/aw-junaid/Kali-Linux/blob/main/Kali%20Linux%20Tools/Images/amass.png)

`amass` is a powerful open-source tool used for performing network mapping and asset discovery. It helps security professionals and researchers discover subdomains, IP addresses, and other related information about a target organization or domain.  It's an essential tool for reconnaissance and vulnerability assessment.

**What `amass` Does:**

`amass` automates the process of gathering information about a target by using various techniques, including:

* **DNS Enumeration:** Querying DNS servers for subdomains (e.g., using zone transfers, brute-forcing, and dictionary attacks).
* **Certificate Transparency Logs:** Searching Certificate Transparency (CT) logs for certificates issued for the target domain, which can reveal subdomains.
* **Brute-forcing:** Trying common subdomain names.
* **Web Scraping:** Scraping websites for links and other information.
* **OSINT (Open Source Intelligence) Techniques:** Gathering information from various public sources.
* **Active Reconnaissance (optional):** Performing active scans (e.g., port scanning) to identify live hosts (use with caution and only when authorized).

**Key Features and Capabilities:**

* **Subdomain Enumeration:**  Discovers subdomains that might not be publicly known.
* **IP Address Discovery:**  Identifies IP addresses associated with the target domain.
* **Asset Discovery:**  Finds other assets related to the target, such as related domains or organizations.
* **Multiple Data Sources:**  Uses various techniques and data sources for comprehensive discovery.
* **Command-Line Interface:**  Easy to use and scriptable.
* **Output Formats:**  Supports various output formats (text, JSON).

**How to Use `amass`:**

1. **Installation:**  `amass` is available as a Go binary and can be installed using `go install github.com/OWASP/Amass/v3/...@latest`.  You may need to set up your Go environment first.  Check the Amass GitHub page for the most up-to-date installation instructions.
...
amass enum -d example.com
```
👉 Performs **passive** enumeration of subdomains for `example.com`.

---

### **2️⃣ Active Subdomain Discovery**
```bash
amass enum -active -d example.com
```
👉 Uses **active enumeration techniques** like DNS resolution, brute force, and web scraping.

---

### **3️⃣ Find Subdomains Using APIs**
```bash
amass enum -d example.com -src
```
👉 Queries OSINT sources like **Shodan, VirusTotal, and Google** (you may need API keys).

---

### **4️⃣ Brute-Force Subdomains**
```bash
amass enum -brute -d example.com
```
👉 Performs **dictionary-based subdomain brute-forcing**.

---

### **5️⃣ Use a Custom Wordlist for Brute Forcing**
```bash
amass enum -brute -w wordlist.txt -d example.com
```
👉 Uses `wordlist.txt` to find **hidden** subdomains.

---

### **6️⃣ Scan Multiple Domains at Once**
```bash
...
mywebsite.net
```
👉 Scans all domains **in batch mode**.

---

### **7️⃣ Find ASN (Autonomous System Numbers) of a Domain**
```bash
amass intel -org "Microsoft"
```
👉 **Finds ASN and IP ranges** used by `Microsoft`.

---

### **8️⃣ Reverse DNS Lookup (Find Subdomains from an IP Range)**
```bash
amass intel -addr 192.168.1.0/24
```
👉 Performs **reverse lookup** to find subdomains in a **specific IP range**.

---

### **9️⃣ Save Output to a File**
```bash
amass enum -d example.com -o results.txt
```
👉 Saves **found subdomains** into `results.txt`.
...
```
👉 Generates a **graph visualization** of discovered **subdomains & relationships**.

---

## **Advanced Usage**
### **🔹 Passive Recon Only (No DNS Queries)**
```bash
amass enum -passive -d example.com
```
👉 Uses **only public sources** (OSINT).

---

### **🔹 Perform Active Recon with a Proxy**
```bash
amass enum -active -proxy socks5://127.0.0.1:9050 -d example.com
```
👉 Uses **Tor or a proxy** for anonymity.

---

### **🔹 Use Multiple APIs (Requires API Keys)**
```bash
amass enum -d example.com -config amass.config
```
...
shodan = YOUR_API_KEY
```
👉 Queries **VirusTotal, Shodan**, and other sources **via API**.

---

**Interpreting the Results:**

`amass` outputs a list of discovered subdomains, IP addresses, and other information related to the target domain.  The JSON output format is helpful for parsing the results programmatically.

**Key Concepts:**

* **DNS Enumeration:**  Gathering information from DNS servers.
* **Certificate Transparency Logs:** Publicly available logs of issued SSL certificates.
* **Brute-forcing:** Trying common subdomain names.
* **OSINT:** Open Source Intelligence.
* **Active Reconnaissance:**  Performing active scans (e.g., port scanning).

**Use Cases:**

* **Reconnaissance:** Gathering information about a target before a penetration test or security assessment.
* **Asset Discovery:**  Identifying all assets related to a target organization.
* **Vulnerability Management:**  Finding potentially vulnerable systems.

**Important Considerations:**

* **Ethical Use:** Only use `amass` on targets that you have explicit permission to test. Unauthorized scanning is illegal and unethical.
* **Active Reconnaissance:** Use the `-active` option with extreme caution and only when explicitly authorized. Active scanning can be easily detected and may be considered hostile activity.
* **Rate Limiting:** Be mindful of rate limits when querying DNS servers or other public resources.  `amass` has some built-in rate limiting, but it's good practice to be respectful of the target's resources.

`amass` is a powerful tool for discovering assets related to a target organization.  It's an essential part of the reconnaissance phase of any security assessment.  However, it's crucial to use it responsibly and ethically.  Always obtain proper authorization before scanning any target.
